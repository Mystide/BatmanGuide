...
(() => {
  ...
  const BUILD_ID = "2026-03-03.6-auto";
  ...
  // Goal: feel instant when you switch devices, without hammering GitHub.
  // - Push debounce: fast (see scheduleAutoSync)
  // - Pull/sync: frequent when visible; even more frequent right after you interact.
  let lastAutoSyncAt = 0;
  let recentActivityUntil = 0;

  const AUTO_SYNC_MIN_MS = 1500;           // hard throttle
  const AUTO_SYNC_FAST_MS = 2500;          // while active
  const AUTO_SYNC_IDLE_MS = 7000;          // while just open/visible
  const AUTO_SYNC_ACTIVE_WINDOW_MS = 90*1000;
  ...
  // Startup sync ASAP
  setTimeout(()=>{ maybeAutoSync("startup"); }, 120);
  ...
  // High-frequency ticker with adaptive cadence
  setInterval(()=>{
