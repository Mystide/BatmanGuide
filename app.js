...
(() => {
  ...
  const BUILD_ID = "2026-03-03.7-auto";
  ...
  // ---- Auto sync (fast, but GitHub-friendly) ----
  // GitHub can rate-limit/abuse-limit if we spam. So:
  // - one sync at a time (inFlight lock)
  // - adaptive polling (fast after activity, slower when idle)
  ...
  const AUTO_SYNC_MIN_MS = 2500;              // hard throttle
  const AUTO_SYNC_FAST_MS = 6000;             // while active
  const AUTO_SYNC_IDLE_MS = 20000;            // while just open/visible
  const AUTO_SYNC_ACTIVE_WINDOW_MS = 60*1000; // after activity

  function markActivity(){
    recentActivityUntil = Date.now() + AUTO_SYNC_ACTIVE_WINDOW_MS;
  }

  function cfgReadyNow(){
    const c = loadCfg();
    return ready(c) ? c : null;
  }

  function shortErr(e){
    const msg = String((e && e.message) ? e.message : e);
    // keep it readable in the pill
  ...
  async function maybeAutoSync(reason){
    ...
    try{
      ...
      // Backoff to avoid hammering GitHub if we hit rate limits / network hiccups.
      backoffMs = backoffMs ? Math.min(backoffMs * 2, 120000) : 8000;
      backoffUntil = Date.now() + backoffMs;
      setSyncStatus(`Auto-sync failed — retry in ${Math.ceil(backoffMs/1000)}s`);
      // Keep the full error available via alert if user clicks Sync manually
  ...
  async function pushOnExit(){
    const c = cfgReadyNow();
    if(!c || !c.auto) return;
    try{ await gistPush(c); }catch(e){}
  }

  // Startup sync ASAP (after first paint)
  setTimeout(()=>{ markActivity(); maybeAutoSync("startup"); }, 250);

  // Sync when returning/using the app
  window.addEventListener("focus", ()=>{ markActivity(); maybeAutoSync("focus"); });
  window.addEventListener("pageshow", ()=>{ markActivity(); maybeAutoSync("pageshow"); });
  document.addEventListener("visibilitychange", ()=>{
    if(document.visibilityState === "visible"){ markActivity(); maybeAutoSync("visible"); }
  });

  // Interaction triggers (helps when the tab was already open on device B)
  document.addEventListener("pointerdown", ()=>{ markActivity(); maybeAutoSync("tap"); }, {passive:true});
  document.addEventListener("keydown", ()=>{ markActivity(); maybeAutoSync("key"); }, {passive:true});
  ...
  setInterval(()=>{
    ...
    const now = Date.now();
    const target = (now < recentActivityUntil) ? AUTO_SYNC_FAST_MS : AUTO_SYNC_IDLE_MS;
    if(now - lastAutoSyncAt >= target){
      maybeAutoSync("poll");
    }
  }, 1000);
