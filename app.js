...
(() => {
  ...
  const BUILD_ID = "2026-03-03.2-debug";
  ...
  async function gistPull(cfg){
    ...
    if(!files[cfg.gistFile] || typeof files[cfg.gistFile].content !== "string") throw new Error(`Gist file not found: ${cfg.gistFile}`);
    const res = importPayload(files[cfg.gistFile].content);
  ...
  async function gistSync(cfg){
    ...
    const data = await ghFetch(cfg, "GET", `https://api.github.com/gists/${cfg.gistId}`);
    const files = data.files || {};
    if(!files[cfg.gistFile] || typeof files[cfg.gistFile].content !== "string"){
      await gistPush(cfg);
      return;
    }
    let remote = null;
    try{ remote = JSON.parse(files[cfg.gistFile].content); }catch{ remote = null; }
