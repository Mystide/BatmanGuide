...
(() => {
  ...
  const BUILD_ID = "2026-03-03.5-auto";
  ...
  // ---- Auto sync (no button presses) ----
  let lastAutoSyncAt = 0;
  const AUTO_SYNC_MIN_MS = 6 * 1000;   // don't spam
  const AUTO_SYNC_INTERVAL_MS = 15 * 1000;
  ...
  async function maybeAutoSync(reason){
    ...
    try{
      await gistSync(c);
      // gistSync already updates status
    }catch(e){
      // Keep status informative but non-blocking
      setSyncStatus("Auto-sync failed");
    }
  }

  // When leaving the page: try to push quickly (best effort)
  ...
  window.addEventListener("focus", ()=>{ maybeAutoSync("focus"); });
  // Sync when you start interacting (useful if the page stays open)
  document.addEventListener("pointerdown", ()=>{ maybeAutoSync("tap"); }, {passive:true});
  document.addEventListener("keydown", ()=>{ maybeAutoSync("key"); }, {passive:true});
  window.addEventListener("pageshow", ()=>{ maybeAutoSync("pageshow"); });
  ...
  // Initial auto-sync (after first paint)
  setTimeout(()=>{ maybeAutoSync("startup"); }, 150);

  setInterval(()=>{
    if(document.visibilityState === "visible") maybeAutoSync("interval");
